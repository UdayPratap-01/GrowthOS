...
## 0c. P2-A — AI creative & campaign engine — 2026-08-11

**Verdict is unchanged: NOT READY for a payment-taking launch, and P2-A does not
change that.** It adds no payment processing and no ad publishing. What it adds is
...
| Capability | Status |
|---|---|
| Campaign strategy (13 sections, evidence preserved) | IMPLEMENTED · TESTED · VERIFIED |
| Structured campaign brief | IMPLEMENTED · TESTED · VERIFIED |
| Copy engine — distinct concepts, not rewordings | IMPLEMENTED · TESTED · VERIFIED |
| Creative concepts with grounded image/video prompts | IMPLEMENTED · TESTED · VERIFIED |
| Real image generation through the existing provider abstraction | IMPLEMENTED · TESTED · **provider-dependent** |
| Real video generation (submit → poll → download → store) | IMPLEMENTED · TESTED · **provider-dependent** |
| Creative variations on a declared axis | IMPLEMENTED · TESTED · VERIFIED |
| Campaign structure (campaign → ad sets → ads) | IMPLEMENTED · TESTED · VERIFIED |
| Approval workflow with recorded decision | IMPLEMENTED · TESTED · VERIFIED |
| Usage metering per generation step | IMPLEMENTED · TESTED · VERIFIED |
| Cost guardrails enforced server-side | IMPLEMENTED · TESTED · VERIFIED |
| Async generation with real progress | IMPLEMENTED · TESTED · VERIFIED |
| Tenant isolation across every new endpoint | IMPLEMENTED · TESTED · VERIFIED |
| Campaign publishing | **NOT IMPLEMENTED — out of scope** |

Architecture, agent-by-agent behaviour, provider contracts and operational notes
are in [`docs/AI_CREATIVE_ENGINE.md`](docs/AI_CREATIVE_ENGINE.md).

### What is REAL, DEMO and NOT_CONFIGURED

The distinction is enforced in data, not in wording. `CreativeAsset.data_source`
is `live` only when a real provider produced the bytes, and the API exposes
`is_real` derived from it; the demo provider stamps `demo` and the UI labels it.

- **REAL** — text generation via `AI_PROVIDER=openai|anthropic`; image generation
  via `IMAGE_PROVIDER=openai`; video generation via `VIDEO_PROVIDER=replicate`;
  storage to any S3-compatible backend; the full job pipeline; approval records.
- **DEMO** — `IMAGE_PROVIDER=demo` writes a genuine PNG to storage and labels the
  asset DEMO. It proves the pipeline, and proves nothing about a vendor's output.
  `AI_PROVIDER=mock` returns pre-written text and is refused in production.
- **NOT_CONFIGURED** — an unset media provider. The stage reports
  `NOT_CONFIGURED`, no job is created, no asset row is written and no URL is
  offered. The campaign still generates its strategy, copy and concepts.

### Verification performed

`apps/api/scripts/verify_p2a_e2e.py` drives the deployed shape of the system:
HTTP requests accept the run, the real `app.worker.Worker` loop drains the queue
out of band, and the generated bytes are read back through the authenticated API
and checked against image magic numbers.

| Configuration | Result |
|---|---|
| PostgreSQL 16, `IMAGE_PROVIDER=demo`, `VIDEO_PROVIDER=none` | 57/57 checks passed |
| PostgreSQL 16, both providers unset | 36/36 checks passed (everything reports `NOT_CONFIGURED`) |
| `alembic upgrade head` → `alembic check` on a fresh PG16 database | no drift |
| `alembic downgrade base` → `upgrade head` round trip | clean |

**No real image or video provider was configured in this environment**
(`IMAGE_PROVIDER=demo`, `VIDEO_PROVIDER=none`, no vendor API keys present), so
this report does **not** claim that image or video generation works against a
...
`alembic check` on the pre-P2-A tree reported permanent drift on six columns in
the billing and usage tables: the models type them as non-optional while the P1
migrations created them nullable. Drift that is always present hides the one
difference that matters during a release, so `a17c5e8b4d90` reconciles it
(backfilling NULLs before applying the constraint) and
`OrganizationSubscription.status` now declares its width so the column stays
`VARCHAR(32)` instead of being narrowed to the longest current member.
`alembic check` is now clean.

### What P2-A deliberately does not do

No advertising money can be spent by this code. There is no publish call, no
budget mutation against a live campaign, and no `PUBLISHED` state — the terminal
state is `READY_TO_PUBLISH`. `campaigns.external_id` is the only evidence a
campaign exists on a platform and nothing in P2-A writes it, so the UI cannot
show a campaign as published without an integration that actually confirmed one.
